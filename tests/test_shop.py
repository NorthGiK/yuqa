...
def test_shop_purchase():
    ...
    item = ShopItem(id=1, sell_resource_type=ResourceType.CRYSTALS, buy_resource_type=ResourceType.COINS, price=50, quantity=3)
...
def test_shop_item_validation():
    with pytest.raises(ValidationError):
        ShopItem(id=1, sell_resource_type=ResourceType.CRYSTALS, buy_resource_type=ResourceType.COINS, price=0, quantity=1)
    with pytest.raises(NotEnoughResourcesError):
        ShopService().purchase(Player(telegram_id=1), ShopItem(id=1, sell_resource_type=ResourceType.CRYSTALS, buy_resource_type=ResourceType.COINS, price=50, quantity=1))
