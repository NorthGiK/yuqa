...
from yuqa.telegram.services.services_support import _next_id


SeasonGetter = Callable[[], Awaitable[BattlePassSeason | None]]
ProgressGetter = Callable[[int], Awaitable[BattlePassProgress]]


if TYPE_CHECKING:

    class _BattlePassServiceMixinBase(TelegramServiceContext):
        """Type-only base for mixin attribute and method completion."""

else:

    class _BattlePassServiceMixinBase:
        """Runtime base without protocol method stubs."""


class BattlePassServiceMixin(_BattlePassServiceMixinBase):
    ...
    async def _active_season(
        self,
        repository: BattlePassSeasonRepositoryLike,
    ) -> BattlePassSeason | None:
    ...
    async def _active_progress_for(
        self,
        repository: BattlePassProgressRepositoryLike,
        season_getter: SeasonGetter,
        player_id: int,
    ) -> BattlePassProgress:
    ...
    async def _buy_next_level(
        ...
        *,
        telegram_id: int,
        season_getter: SeasonGetter,
        progress_getter: ProgressGetter,
        progress_repository: BattlePassProgressRepositoryLike,
