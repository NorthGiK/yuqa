...
def test_clan_lifecycle():
    ...
    clan = Clan(
        id=10,
        owner_player_id=leader.telegram_id,
        name="Alpha",
        icon="icon.png",
        min_entry_rating=1000,
    )
    service = ClanService()
    service.create_clan(clan, leader)
    service.join_clan(clan, member)
    assert (
        leader.clan_id == clan.id
        and member.clan_id == clan.id
        and leader.wallet.coins == 10_000
    )
...
def test_blacklist_management():
    ...
    clan = Clan(
        id=10, owner_player_id=leader.telegram_id, name="Alpha", icon="icon.png"
    )
    service = ClanService()
