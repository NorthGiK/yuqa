...
from src.cards.domain.entities import Ability, CardTemplate, PlayerCard
...
if TYPE_CHECKING:
    ...
    class _ContentAdminServiceMixinBase(TelegramServiceContext):
        """Type-only base for mixin attribute and method completion."""

else:

    class _ContentAdminServiceMixinBase:
        """Runtime base without protocol method stubs."""


class ContentAdminServiceMixin(_ContentAdminServiceMixinBase):
    ...
    async def create_card_template(
        ...
        async with self.write_transaction():
            universe_value = universe.value if isinstance(universe, Universe) else universe
            await self.add_universe(universe_value)
        ...
        async with self.write_transaction():
            ...
            for banner in list(self.banners.items.values()):
                banner.pools = [
                    item
                    for item in banner.pools
                    if item.card_template_id != template_id
                ]
                await self.banners.save(banner)
            await self.set_standard_cards(
                [
                    item
                    for item in await self.list_standard_cards()
                    if item != template_id
                ]
            )
    ...
    def _read_standard_cards(self) -> list[int]:
        ...
        if self.catalog is None:
            return list(self._standard_cards)
        return list(self.catalog.standard_cards)
